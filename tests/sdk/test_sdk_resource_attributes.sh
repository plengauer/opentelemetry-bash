...
echo "$data"
assert_equals "shell" $(echo "$data" | jq -r '.resource.attributes."telemetry.sdk.language"')
...
assert_equals "$SHELL" $(echo "$data" | jq -r '.resource.attributes."process.executable.name"')
...
assert_equals "$SHELL" $(echo "$data" | jq -r '.resource.attributes."process.runtime.name"')
