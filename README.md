...
| OTEL_SHELL_CONFIG_MUTE_INTERNAL             | `TRUE`, `FALSE` | `FALSE`                                      | Only create `SERVER`, `CONSUMER`, `CLIENT` and `PRODUCER` spans, mute all `INTERNAL` spans. | stable       |
| OTEL_SHELL_CONFIG_MUTE_BUILTIN              | `TRUE`, `FALSE` | `TRUE` for GitHub Actions, `FALSE` otherwise | Do not create spans for built-in commands like `echo`, `printf`, `[`, ...                   | stable       |
