...
### Span Attributes
Customize your spans by manually setting span attributes with the span handle. This is only possible between `otel_span_start` and `otel_span_end` for the respective span handle. Valid types are `string`, `int`, and `float`, and `auto`. The `auto` type will try to guess the type based on the value.
```bash
...
## Metrics
To record metrics, first create a new metric with a key to get a metric handle. The returned metric handle is used to customize the metric data point and record its value. Valid types are `string`, `int`, and `float`, and `auto`. The `auto` type will try to guess the type based on the value.
```bash
