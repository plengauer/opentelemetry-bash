#/bin/bash
set -e
if [ -z "${INPUT_WORKFLOW_RUN_ID:-}" ] && [ -z "${INPUT_WORKFLOW_RUN_ATTEMPT:-}" ]; then exec bash ./main_legacy.sh; fi
...
github repos/"$GITHUB_REPOSITORY"/actions/runs/"$INPUT_WORKFLOW_RUN_ID"/attempt/"$INPUT_WORKFLOW_RUN_ATTEMPT"/jobs | jq -r '.jobs[] | [.id, .conclusion, .started_at, .completed_at, .name] | @csv' | while IFS=, read -r job_id job_conclusion job_started_at job_completed_at job_name; do
  ...
  github repos/"$GITHUB_REPOSITORY"/actions/runs/"$INPUT_WORKFLOW_RUN_ID"/attempt/"$INPUT_WORKFLOW_RUN_ATTEMPT"/jobs/"$job_id" | jq -r '.steps[] | [.conclusion, .started_at, .completed_at, .name] | @csv' | while IFS=, read -r step_conclusion step_started_at step_completed_at step_name; do
