...
sudo docker run --detach --network=host --mount type=bind,source="$(pwd)"/collector.yaml,target=/etc/otelcol-contrib/config.yaml "$OTEL_SHELL_COLLECTOR_IMAGE"
sleep 10
sudo docker logs "$(sudo docker container ls -q)"
