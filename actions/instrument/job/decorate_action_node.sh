...
if [ "$(cat /tmp/opentelemetry_shell_action_name 2> /dev/null)" = "$GITHUB_ACTION" ] || [ "$(cat /proc/$PPID/cmdline | tr '\000-\037' ' ' | cut -d ' ' -f 1 | rev | cut -d / -f 1 | rev)" != "Runner.Worker" ]; then exec "$@"; fi
...
exit_code="$?"
if [ "$exit_code" -ne 0 ]; then otel_span_error "$span_handle"; touch /tmp/opentelemetry_shell.github.error; fi
