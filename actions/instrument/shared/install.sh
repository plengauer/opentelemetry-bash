...
if [ -n "$action_tag_name" ]; then
  if [ "$INPUT_CACHE" = "true" ]; then
    ...
    sudo -E -H node -e "require('@actions/cache').restoreCache(['/tmp/opentelemetry-shell.deb', '/var/cache/apt/archives/*.deb', '$(sudo pip cache dir)', '$(sudo npm config get cache)'], '$cache_key');"
  fi
  ...
  if ! [ -f "$debian_file" ]; then
    ...
    [ -z "${cache_key:-}" ] || sudo -E -H node -e "require('@actions/cache').saveCache(['/tmp/opentelemetry-shell.deb', '/var/cache/apt/archives/*.deb', '$(sudo pip cache dir)', '$(sudo npm config get cache)'], '$cache_key');"
  fi
