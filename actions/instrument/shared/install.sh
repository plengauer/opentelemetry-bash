...
if [ -n "$action_tag_name" ]; then
  ...
  if [ "$INPUT_CACHE" = "true" ]; then
    ...
    node -e "require('@actions/cache').restoreCache(['$packages_cache_directory', '/var/cache/apt/archives/*.deb', '$(sudo pip cache dir)', '$(sudo npm config get cache)'], '$cache_key');"
  fi
  ...
  if ! [ -f "$debian_file" ]; then
    ...
    [ -z "${cache_key:-}" ] || node -e "require('@actions/cache').saveCache(['$packages_cache_directory', '/var/cache/apt/archives/*.deb', '$(sudo pip cache dir)', '$(sudo npm config get cache)'], '$cache_key');"
  fi
