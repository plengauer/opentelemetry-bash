...
if [ "$GITHUB_REPOSITORY" = "$GITHUB_ACTION_REPOSITORY" ] && dpkg -l | grep -q opentelemetry-shell && false; then
  ...
  [ "$INPUT_CACHE" != "true" ] || node -e "require('@actions/cache').restoreCache(['$packages_cache_directory', '/var/cache/apt/archives/*.deb', '/home/runner/.cache/pip', '/home/runner/.npm'], '$cache_key');"
  ...
  if ! [ -f "$debian_file" ]; then
    ...
    [ "$INPUT_CACHE" != "true" ] || node -e "require('@actions/cache').saveCache(['$packages_cache_directory', '/var/cache/apt/archives/*.deb', '/home/runner/.cache/pip', '/home/runner/.npm'], '$cache_key');"
  fi
