...
if [ "${OTEL_LOGS_EXPORTER:-otlp}" = otlp ] && [ -z "${OTEL_EXPORTER_OTLP_LOGS_ENDPOINT:-${OTEL_EXPORTER_OTLP_ENDPOINT:-}}" ]; then
  echo "::error ::OpenTelemetry for GitHub actions is configured to export logs, but no endpoint is configured." && false
fi
if [ "${OTEL_METRICS_EXPORTER:-otlp}" = otlp ] && [ -z "${OTEL_EXPORTER_OTLP_METRICS_ENDPOINT:-${OTEL_EXPORTER_OTLP_ENDPOINT:-}}" ]; then
  echo "::error ::OpenTelemetry for GitHub actions is configured to export metrics, but no endpoint is configured." && false
fi
if [ "${OTEL_TRACES_EXPORTER:-otlp}" = otlp ] && [ -z "${OTEL_EXPORTER_OTLP_TRACES_ENDPOINT:-${OTEL_EXPORTER_OTLP_ENDPOINT:-}}" ]; then
  echo "::error ::OpenTelemetry for GitHub actions is configured to export traces, but no endpoint is configured." && false
fi
