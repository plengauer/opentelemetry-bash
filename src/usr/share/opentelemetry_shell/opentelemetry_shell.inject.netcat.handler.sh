...
if \[ "$OTEL_SHELL_CONFIG_NETCAT_ASSUME_REQUEST_RESPONSE" = TRUE ]; then
  ...
  (\timeout "${OTEL_SHELL_CONFIG_NETCAT_READ_TIMEOUT:-0}" \cat "$span_handle_file_0" | \tee "$span_handle_file_1" | \tee "$span_handle_file_2" > /dev/null &)
  _otel_netcat_parse_request 1 "$span_handle_file_0" $netcat_command_string | { unset OTEL_SHELL_CONFIG_NETCAT_READ_TIMEOUT; span_handle="$(\cat "$span_handle_file_1")"; if \[ -n "$span_handle" ]; then otel_span_activate "$span_handle"; fi; \eval "$handler_command_string" || \echo "$?" > "$exit_code_file"; } | _otel_netcat_parse_response 1 "$span_handle_file_2"
