...
if \[ "$OTEL_SHELL_CONFIG_NETCAT_ASSUME_REQUEST_RESPONSE" = TRUE ]; then
  ...
  OTEL_SHELL_CONFIG_NETCAT_INIT_TIMEOUT=60
  _otel_netcat_parse_request 1 "$span_handle_file_0" $netcat_command_string | { unset OTEL_SHELL_CONFIG_NETCAT_INIT_TIMEOUT; otel_span_activate "$(\cat "$span_handle_file_1")"; \eval "$handler_command_string" || \echo "$?" > "$exit_code_file"; } | _otel_netcat_parse_response 1 "$span_handle_file_2"
