...
_otel_auto_instrument() {
  local hint="$1"
\echo "DEBUG auto instrument $hint $(\alias | \wc -l) $PATH $OTEL_SHELL_SPAN_ATTRIBUTES_OVERRIDE"
  local IFS=' 
'
...
\echo "DEBUG auto instrument using cache $(\alias | \wc -l)"
...
\echo "DEBUG auto instrumented $(\alias | \wc -l)"
}
