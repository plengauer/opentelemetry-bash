...
\dpkg -L opentelemetry-shell >&2
    for kvp in $(\printenv | \grep '^OTEL_' | \cut -d = -f 1); do \echo -n ' '; _otel_escape_args --env "$kvp"; done
    for file in $(\dpkg -L opentelemetry-shell | \grep -vE '^usr$' | \grep -vE '^usr/share$' | \grep -vE '^opt$'); do \echo -n ' '; _otel_escape_args --mount type=bind,source="$file",target="$file",readonly; done
