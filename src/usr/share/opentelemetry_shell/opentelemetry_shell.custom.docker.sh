...
_otel_inject_docker() {
  ...
  OTEL_LOGS_EXPORTER="${otel_logs_exporter:-$OTEL_LOGS_EXPORTER}" OTEL_METRICS_EXPORTER="${otel_metrics_exporter:-$OTEL_METRICS_EXPORTER}" OTEL_TRACES_EXPORTER="${otel_traces_exporter:-$OTEL_TRACES_EXPORTER}" \eval _otel_call "$(_otel_inject_docker_args "$@")"
}
