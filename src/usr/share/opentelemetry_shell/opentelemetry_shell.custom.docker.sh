...
_otel_inject_docker_args() {
  ...
  if \[ "$command" = run ] && \docker run --rm --entrypoint cat "$image" /etc/os-release | \grep -E '^NAME=' | \grep -qE 'Debian|Ubuntu|Alpine Linux'; then
    ...
    \echo -n ' '; _otel_escape_args --mount type=bind,source="$_otel_remote_sdk_pipe",target=/opt/opentelemetry_shell/"$_otel_remote_sdk_pipe"
    \echo -n ' '; _otel_escape_args --env OTEL_REMOTE_SDK_PIPE=/opt/opentelemetry_shell/"$_otel_remote_sdk_pipe"
