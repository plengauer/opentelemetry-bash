...
_otel_inject_java() {
  ...
  if \[ "$OTEL_SHELL_CONFIG_INJECT_DEEP" = TRUE ] && \[ -r /opt/opentelemetry_shell/rootcontextagent.jar ] && \[ -r /opt/opentelemetry_shell/opentelemetry-javaagent.jar ] && \[ "$(_otel_call "$1" --version | \head -n 1 | \cut -d ' ' -f 2 | \cut -d . -f 1)" -ge 8 ]; then
