...
_otel_inject_java() {
  ...
  if [ -r /opt/opentelemetry_shell/rootcontextagent.jar ] && [ -r /opt/opentelemetry_shell/opentelemetry-javaagent.jar ] && [ "$("$1" --version | head -n 1 | cut -d ' ' -f 2 | cut -d . -f 1)" -ge 8 ]; then
