...
_otel_propagate_curl() {
  ...
  local stderr_pid="$!"
  local exit_code=0
  _otel_call "$@" -H "traceparent: $OTEL_TRACEPARENT" -v --no-progress-meter 2> "$stderr_pipe" || exit_code="$?"
  \wait "$stderr_pid"
  \rm "$stderr_pipe"
...
_otel_pipe_curl_stderr() {
  ...
  while read -r line; do
    ...
    if \[ -z "$span_handle" ] && \[ -n "$host" ] && \[ -n "$ip" ] && \[ -n "$port" ] && _otel_string_starts_with "$line" "> " && \[ "$is_receiving" = 1 ]; then
      ...
      otel_span_attribute_typed "$span_handle" string url.path="/$(\printf '%s' "$path" | \cut -d ? -f 1)"
