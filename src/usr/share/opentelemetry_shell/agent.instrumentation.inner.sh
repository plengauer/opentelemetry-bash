...
'
  ...
  while \[ "$#" -gt 0 ] && ( _otel_string_starts_with "$1" - || ! ( \[ -x "$1" ] || \[ -x "$(\which "$1" 2> /dev/null)" ] ) || ( \[ -f "$1" ] && _otel_string_starts_with "$(_otel_resolve_shebang "$1")" "$command " ) ); do \echo -n " "; _otel_escape_arg "$1"; shift; done
