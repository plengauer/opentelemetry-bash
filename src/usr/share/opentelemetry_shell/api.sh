...
if \[ -p "$_otel_remote_sdk_pipe" ]; then
  ...
  otel_init() {
    ...
    ( (\env $extra_env_flags /opt/opentelemetry_shell/venv_sdk/bin/python /usr/share/opentelemetry_shell/sdk.py "shell" "$(_otel_package_version opentelemetry-shell)" < "$_otel_remote_sdk_pipe" 1> "$sdk_output" 2> "$sdk_output") &)
    \exec 7> "$_otel_remote_sdk_pipe"
...
'
  ...
  if \[ "$command_type" = file ]; then
    local executable_path="$(_otel_string_contains "$command_name" / && \echo "$command_name" || \[ "$_otel_shell" = bash ] && \type -P "$command_name" 2> /dev/null || \which "$command_name")"
