...
_otel_inject_shell_args_with_copy() {
  ...
  \echo -n "OTEL_SHELL_AUTO_INSTRUMENTATION_HINT=\"$temporary_script\"; . otel.sh; \set -- \"$(_otel_escape_args "$@")\"; " > "$temporary_script"
...
_otel_inject_shell_with_copy() {
  ...
  local exit_code=0
  OTEL_SHELL_AUTO_INJECTED=TRUE \eval _otel_call "$(_otel_inject_shell_args_with_copy "$temporary_script" "$@")" || local exit_code=$?
