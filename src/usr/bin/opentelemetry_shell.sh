...
otel_list_alias_commands() {
  \alias | \cut -d' ' -f2- | \sed "s/^'\(.*\)'$/\1/" | while read alias_entry; do #   # TODO we should sed somewhere! and in sh there is no alias prefix so the split by whitespace looks fishy!
