...
otel_alias_prepend() {
  ...
  local previous_command="$(\alias $1 2> /dev/null | \cut -d= -f2- | \tr -d \')"
  ...
  local previous_otel_command="$(\echo "$previous_command" | \tr ' ' '\n' | \grep '^otel_' | \tr '\n' ' ')"
  local previous_alias_command="$(\echo "$previous_command" | \tr ' ' '\n' | \grep -v '^otel_' | \tr '\n' ' ')"
